...
const About = () => {
  ...
  useEffect(() => {
    ...
    if (prefersReduced || onMobile) {
      // Lazy reveal calendar when in view to save work on mobile
  ...
  return (
    ...
    >
      <h2
        style={{ color: "#1e293b", fontSize: "2.5rem", marginBottom: "30px" }}
      >
        Hakkımda
      </h2>
      <div
      ...
      >
        <div className="profile-flip-card" onClick={handleFlip} tabIndex={0}>
          <div className={`profile-flip-inner${isFlipped ? " flipped" : ""}`}>
            <div className="profile-flip-front">
              <img
                ...
                onError={(e) => {
                  if (
                    !e.currentTarget.src.includes("placeholder-profile.png")
                  ) {
        ...
        <div className="about-text">
          <p>
            Merhaba! Ben Enes. Teknolojiye ve yeniliğe büyük bir tutkuyla bağlı
            bir yazılım geliştiricisiyim. Problem çözmeyi, yeni şeyler öğrenmeyi
            ve bildiklerimi paylaşmayı seviyorum. Modern ve kullanıcı dostu web
            uygulamaları geliştirmek en büyük ilgi alanım.
          </p>
          <p>
            Kariyerim boyunca çeşitli projelerde yer alarak hem ön yüz
            (front-end) hem de arka yüz (back-end) teknolojilerinde deneyim
            kazandım. Amacım, insanların hayatını kolaylaştıran ve onlara değer
            katan ürünler ortaya çıkarmak.
          </p>
        </div>
      ...
      <div className="github-contributions-section" ref={calendarRef}>
        <div className="github-card">
          <h3 className="github-title">GitHub Katkı Grafiğim</h3>
          <div className="github-calendar-wrapper">
            {showCalendar ? (
              <Suspense fallback={<div style={{padding:10}}>Yükleniyor…</div>}>
                <GitHubCalendar
              ...
              <div style={{height: 140, width: '100%', display:'flex', alignItems:'center', justifyContent:'center', color:'#64748b'}}>Grafik hazır…</div>
            )}
          </div>
        </div>
      </div>
